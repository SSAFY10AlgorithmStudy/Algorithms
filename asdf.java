...
public class asdf {
    ...
    private static void M1_노래부를순서정하기_순열(int cnt, int flag) {
        ...
        for (int i = 0; i < N; i++) { // 배열 순회
            if ((flag & 1 << i) != 0) // 이름이 선택 됐는지 확인
                continue;
            result[cnt] = input[i];
    ...
    private static void M5_노래부를팀만들기_부분집합() {        
        for (int i = 0; i < (1<<N); i++) { // N 크기짜리 boolean[] -> bit로 대체
            for (int j = 0; j < N; j++) { // boolean 배열 (i) 돌면서 뽑은거 확인
                if((i & (1<<j)) !=0) { // 비트연산으로 뽑힌지 확인 1=뽑고 0=안뽑고
                    // ++totalCnt;
                    sb.append(input[j]).append(" "); 
                }
            }
            sb.append("\n"); // 배열 돌기 끝나고 다음줄 -> 다음 집합으로 넘어감
        }
    }
