...
public class PermutationNPTest {
	...
	public static void main(String[] args) {
		...
		int N = sc.nextInt();
		int[] input = new int[N];  // 재귀와 다르게 입력받은 수를 가지고 위치 변경을 통해 만든다.
		
		for(int i=0; i<N; i++) {
			input[i] = sc.nextInt();
		}
		
		Arrays.sort(input); //(중요) 오름차순의 형태로 정렬
	...
	private static boolean np(int[] p) { // p : 다음 순열을 원하는 기존 순열의 배열
		...
		int N = p.length;
		int i = N-1;  // 제일 뒤부터 탐색
		while(i>0 && p[i-1]>=p[i]) --i;  // 같거나 크면 0위치 전까지 앞으로 계속 감
